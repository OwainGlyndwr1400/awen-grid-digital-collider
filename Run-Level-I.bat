@echo off
title Awen Grid Digital Collider - Level I Run (1M nodes x 369 ticks)
...
%PY% -m awen_collider.run_collider --nodes 1000000 --ticks 369 --collide-every 9 --json logs\level1_1M.json --html logs\level1_1M_report.html

echo.
echo Opening graphical run report + live dashboard...
start "" "logs\level1_1M_report.html"
